package com.example.ecoffe;

public class Infomation {

    final static public int Nothing=-1;

    final static public int ConnectFail=0;
    final static public int ConnectSuccess=1;

    final static public int Tumbler=2;
    final static public int PaperCup=3;

    final static public int Americano=2000;
    final static public int CaffeLatte=2500;
    final static public int GreenLatte=3000;

    final static public int PayFail=4;
    final static public int PaySuccess=5;
    final static public int PayUseCoupon=6;

    final static public int Finish=7;

}
